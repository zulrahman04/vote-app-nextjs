...
export default function CandidateForm(props : Props){
    ...
    return (        
        <div className="flex flex-col border border-zinc-100 p-5">
            ...
            <h1 className="flex w-1/2 bg-zinc-100 aspect-square justify-center items-center self-center text-4xl rounded-full">
                {props.candidate.key}
            </h1>
            <label className="text-sm mt-3 mb-1">Nama Kandidat</label>
            <Form placeholder="Masukkan Nama" value={candidate.name} onChange={(e)=>{setCandidate({...candidate, name:e})}}/>
        </div>
    )
}